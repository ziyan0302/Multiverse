...
def white_box_attack(input_feature, label, model_func, config,
                     grid_obs_labels, grid_obs_regress,
                     obs_length, pred_length, norm_feat=False):
  """
    input_feature, [N*T_obs, SH, SW, SC],
    label, [N, T_pred]
  """

  def create_random_target(label_, max_class_):
    label_offset = tf.random_uniform(
        tf.shape(label_), minval=1, maxval=max_class_, dtype=tf.int32)
    # so label will be between the correct class range
    return tf.floormod(
        label_ + label_offset, tf.constant(max_class_, tf.int32))


  def get_start_adv(feature):
    # start from clean or random noise?
    init_start = tf.random_uniform(tf.shape(feature),
                                   minval=-config.adv_epsilon,
                                   maxval=config.adv_epsilon, dtype="float32")
    if config.adv_start_from_clean_prob >= 1.0:
      return feature
    if config.adv_start_from_clean_prob > 0:
      start_from_clean = tf.cast(tf.greater(
          tf.random_uniform(shape=[]), config.adv_start_from_clean_prob),
                                 tf.float32)
      init_start = init_start * start_from_clean

    return feature + init_start

  def one_step_attack(adv_input):
    batch_size = config.batch_size

    pred_class, pred_reg = model_func(
        adv_input, batch_size, grid_obs_labels,
        grid_obs_regress, obs_length, pred_length)

    #[N*T_pred]
    label = tf.reshape(target_label, [-1])

    # model output
    # [N, T, H, W, 1]
    this_pred_class = pred_class[target_scale_idx]
    # [N*T, H*W]
    this_pred_class = tf.reshape(this_pred_class, [-1, h * w])

    classification_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=label,
        logits=this_pred_class)

    # gradient w.r.t the input
    gradient, = tf.gradients(classification_loss, adv_input)

    # we want to minimize the classification loss (w.r.t to the random target
    # label) so we minus the sign gradient and epsilon
    # FGSM
    if config.adv_use_fgsm:
      adv_input = adv_input - config.adv_epsilon * tf.sign(gradient)
    else:
      # PGD
      adv_input = adv_input - tf.sign(gradient) * config.adv_step_size
    adv_input = tf.clip_by_value(adv_input, lower_bound, upper_bound)
    return adv_input

  target_scale_idx = config.use_grids.index(True)
  h, w = config.scene_grids[target_scale_idx]
  max_grid_class = int(h * w)
  label = tf.cast(label, tf.int32)
  # targeted attack
  target_label = create_random_target(label, max_grid_class)

  lower_bound = tf.clip_by_value(input_feature - config.adv_epsilon, -1., 1.)
  upper_bound = tf.clip_by_value(input_feature + config.adv_epsilon, -1., 1.)

  # add random noise to input feature
  start_adv = get_start_adv(input_feature)

  if norm_feat:
    start_adv = tf.nn.softmax(start_adv)
  ...
  if config.adv_use_fgsm:
    ...
    adv_final = tf.while_loop(
        lambda _: True,
        one_step_attack,
        # we can't use xla due to the use of tf.nn.raw_rnn
        #lambda adv: xla.compile(lambda: one_step_attack(adv))[0],
        [start_adv],
        back_prop=False,
        maximum_iterations=config.adv_num_iter,
        parallel_iterations=1)

  if config.use_mixup:
    dist = tf.distributions.Beta(config.mixup_alpha, config.mixup_alpha)
    weight = dist.sample()

    if config.mixup_mix_adv:
      # mix two adversarial image
      assert config.adv_use_fgsm
      assert config.adv_start_from_clean_prob < 1.0
      start_adv2 = get_start_adv(input_feature)
      adv_final2 = one_step_attack(start_adv2)

      adv_final = adv_final2 * weight + adv_final * (1 - weight)
    else:
      # mix with clean image
      adv_final = input_feature * weight + adv_final * (1 - weight)
...
class Model(object):
  ...
  def __init__(self, config, scope):
    ...
    if config.multiview_train:
      self.obs_scene_extra = tf.placeholder(
          "int32", [N, config.multiview_max_num, None], name="obs_scene_extra")

      self.grid_obs_labels_extra = []
      self.grid_pred_labels_T_extra = []
      self.grid_pred_regress_extra = []
      self.grid_obs_regress_extra = []

      #self.grid_pred_regress = []
      #self.grid_obs_regress = []  # [N, max_num, T, H, W, 2]

    for h, w in config.scene_grids:
      # [N, seq_len]
      self.grid_obs_labels.append(
          tf.placeholder("int32", [N, None], name="grid_obs_classes"))
      ...
      if config.is_train and config.multiview_train:
        self.grid_obs_labels_extra.append(
            tf.placeholder(
                "int32", [N, config.multiview_max_num, None],
                name="grid_obs_classes_extra"))
        self.grid_pred_labels_T_extra.append(
            tf.placeholder(
                "float32", [N, config.multiview_max_num, None],
                name="grid_pred_classes_extra"))
        self.grid_pred_regress_extra.append(
            tf.placeholder("float32",
                           [N, config.multiview_max_num, None, h, w, 2],
                           name="grid_pred_regress_extra"))
        self.grid_obs_regress_extra.append(
            tf.placeholder("float32",
                           [N, config.multiview_max_num, None, h, w, 2],
                           name="grid_obs_regress_extra"))


    self.beam_outputs = None
    self.loss = None

    with tf.variable_scope("person_pred", reuse=tf.AUTO_REUSE) as top_scope:
      self.top_scope = top_scope

      # add scene conv to self
      # [N, T, SH, SW, SC]
      obs_scene = tf.nn.embedding_lookup(
          self.scene_feat, self.obs_scene)
      # [N*T, SH, SW, SC], each channel is 1 or 0
      # float32
      obs_scene = tf.reshape(
          obs_scene, [-1, config.scene_h, config.scene_w, config.scene_class])

      if config.norm_input:
        # normalize images to [-1, 1] for adv learning
        obs_scene = obs_scene * 2.0 - 1.0

      obs_scene_ori = obs_scene

      # adversarial learning by augmenting the input features
      if config.is_train and config.adv_train:
        assert sum(config.use_grids) == 1, "only one scale for adv train"
        target_scale_idx = config.use_grids.index(True)

        obs_scene, target_label = white_box_attack(
            obs_scene, self.grid_pred_labels_T[target_scale_idx],
            self.build_tower, config, self.grid_obs_labels,
            self.grid_obs_regress, self.obs_length, self.pred_length,
            norm_feat=self.config.norm_feat)


        obs_scene = tf.stop_gradient(obs_scene)

      elif config.is_train and config.multiview_train:
        assert sum(config.use_grids) == 1, "only one scale for multiview train"

        obs_scene = self.multiview_augmentation(obs_scene)


        obs_scene = tf.stop_gradient(obs_scene)

      if config.is_train and config.standard_aug:
        # standard random data augmentation, pixel jittering
        # start from clean or random noise?
        pixel_jitter = tf.random_uniform(tf.shape(obs_scene),
                                         minval=-config.adv_epsilon,
                                         maxval=config.adv_epsilon,
                                         dtype="float32")
        obs_scene = tf.cond(
            self.is_train,
            lambda: obs_scene + pixel_jitter,
            lambda: obs_scene_ori)

        obs_scene = tf.stop_gradient(obs_scene)

      pred_class, pred_reg = self.build_tower(
          obs_scene, config.batch_size, self.grid_obs_labels,
          self.grid_obs_regress, self.obs_length, self.pred_length,
          mixup=(config.is_train and config.multiview_train))

      # output
      self.grid_pred_decoded = pred_class
      self.grid_pred_reg_decoded = pred_reg

    if config.is_train:
      self.build_loss()

  def gather_at_second_dim(self, feature, indices):
    # [N] -> indices
    # [N, M, ...] -> features
    feat_indices = tf.stack(
        [tf.range(feature.get_shape()[0]), indices], axis=1)
    # [N, 2] -> [N, T, H, W, C]
    return tf.gather_nd(feature, feat_indices)

  def multiview_augmentation(self, obs_scene):

    target_scale_idx = self.config.use_grids.index(True)
    ...
    def one_step_attack(input_feature, target_label,
                        batch_size, grid_obs_labels, grid_obs_regress,
                        obs_length, pred_length):
      """
        Assume starts with input feature.
      """
      config = self.config
      # input_feature: [N*T_obs, H, W, C]
      input_feature = get_start_adv(input_feature)

      pred_class, pred_reg = self.build_tower(input_feature,
          batch_size, grid_obs_labels, grid_obs_regress,
          obs_length, pred_length)
      ...
      N, _, H, W = this_pred_class.get_shape()[:-1]
      # [N*T, H*W]
      this_pred_class = tf.reshape(this_pred_class, [-1, H * W])
      ...
      # gradient w.r.t the input
      gradient, = tf.gradients(classification_loss, input_feature)

      # we want to minimize the classification loss (w.r.t to the random target
      # label) so we minus the sign gradient and epsilon
      # FGSM
      adv_feature = input_feature - config.adv_epsilon * tf.sign(gradient)

      lower_bound = tf.clip_by_value(input_feature - config.adv_epsilon,
                                     -1., 1.)
      upper_bound = tf.clip_by_value(input_feature + config.adv_epsilon,
                                     -1., 1.)
      adv_feature = tf.clip_by_value(adv_feature, lower_bound, upper_bound)

      return adv_feature, \
          tf.reduce_mean(tf.reshape(classification_loss, [N, -1]), axis=1)

    config = self.config
    M = config.multiview_max_num

    # [N, T_pred]
    ori_label = tf.cast(self.grid_pred_labels_T[target_scale_idx], tf.int32)

    # obs_scene [N*T, H, W, C]
    H, W, C = obs_scene.get_shape()[-3:]
    obs_scene = tf.reshape(obs_scene, [config.batch_size, -1, H, W, C])
    obs_scene_tile = tf.tile(tf.expand_dims(obs_scene, axis=1),
                             [1, M, 1, 1, 1, 1])
    # [N*M*T, H, W, C]
    obs_scene_tile = tf.reshape(obs_scene_tile, [-1, H, W, C])

    obs_length_tile = tf.reshape(
        tf.tile(tf.expand_dims(self.obs_length, axis=1), [1, M]),
        [config.batch_size*M])
    pred_length_tile = tf.reshape(
        tf.tile(tf.expand_dims(self.pred_length, axis=1), [1, M]),
        [config.batch_size*M])

    # also tile the original obs grid and stuff
    grid_obs_labels_tile = []
    grid_obs_regress_tile = []
    for i, (h, w) in enumerate(config.scene_grids):
      # [N, seq_len]
      grid_obs_labels_tile.append(tf.reshape(
          tf.tile(tf.expand_dims(self.grid_obs_labels[i], axis=1), [1, M, 1]),
          [config.batch_size*M, -1]))
      grid_obs_regress_tile.append(tf.reshape(
          tf.tile(tf.expand_dims(self.grid_obs_regress[i], axis=1),
                  [1, M, 1, 1, 1, 1]),
          [config.batch_size*M, -1, h, w, 2]))
     # [N, M, T_pred]
    extra_pred_label = tf.cast(
        self.grid_pred_labels_T_extra[target_scale_idx], tf.int32)

    # [N*M*T, H, W, C], [N*M]
    adv_out, adv_loss = one_step_attack(
        obs_scene_tile, extra_pred_label, config.batch_size*M,
        grid_obs_labels_tile, grid_obs_regress_tile,
        obs_length_tile, pred_length_tile)

    # [N, M]
    adv_loss = tf.reshape(adv_loss, [config.batch_size, M])
    # [N, M, T, H, W, C]
    adv_out = tf.reshape(adv_out, [config.batch_size, M, -1, H, W, C])

    if config.multiview_exp == 1:
      # select top two adv sample for training
      # [N, M]
      loss_val, loss_indices = tf.nn.top_k(adv_loss, k=M, sorted=True)
      adv_feat1 = self.gather_at_second_dim(adv_out, loss_indices[:, 0])
      adv_feat2 = self.gather_at_second_dim(adv_out, loss_indices[:, 1])
    elif config.multiview_exp == 4:
      # select top two adv sample for training
      # [N, M]
      loss_val, loss_indices = tf.nn.top_k(adv_loss, k=M, sorted=True)
      adv_feat1 = self.gather_at_second_dim(adv_out, loss_indices[:, M-1])
      adv_feat2 = self.gather_at_second_dim(adv_out, loss_indices[:, M-2])
    elif config.multiview_exp == 2:
      # select two sample with uniform random
      random_1 = tf.random.uniform(
          [config.batch_size], minval=0, maxval=int(M), dtype=tf.int32)
      random_2 = tf.random.uniform(
          [config.batch_size], minval=1, maxval=int(M), dtype=tf.int32)
      # select random 1 then use random 2
      # + the offset to get without replacement sample
      random_2 = tf.floormod(
          random_1 + random_2, tf.constant(M, tf.int32))

      adv_feat1 = self.gather_at_second_dim(adv_out, random_1)
      adv_feat2 = self.gather_at_second_dim(adv_out, random_2)
    elif config.multiview_exp == 3:

      if config.multiview_use_adv_for_loss:
        adv_out = tf.reshape(adv_out, [-1, H, W, C])
        _, adv_loss = one_step_attack(
            adv_out, extra_pred_label, config.batch_size*M,
            grid_obs_labels_tile, grid_obs_regress_tile,
            obs_length_tile, pred_length_tile)
      ...
      # [N], higher loss, higher weight
      focal_loss_weight = (1.0 - tf.exp(-loss_val[:, 0]))**config.fl_gamma
      self.focal_loss_weight = tf.stop_gradient(focal_loss_weight)
      # use the loss indices to select other view's feature for mixup
      # [N*T_obs, H, W, C]
      #adv_feat1 = obs_scene
      #adv_feat1 = tf.reshape(adv_feat1, [config.batch_size, -1, H, W, C])

      adv_feat1 = self.gather_at_second_dim(adv_out, loss_indices[:, 0])

      # get the other view's feature
      # [N, M, T_obs, H, W, C], each channel is 1 or 0
      obs_scene_extra = tf.nn.embedding_lookup(
          self.scene_feat, self.obs_scene_extra)
      # select the highest loss view's feature
      select_indices = loss_indices[:, 0]
      if config.multiview_random:
        select_indices = tf.random.uniform(
            [config.batch_size], minval=0, maxval=int(M), dtype=tf.int32)

      adv_feat2 = self.gather_at_second_dim(obs_scene_extra, select_indices)

      # remember the choice for mixing up labels [N]
      self.selected_extra_indices = select_indices
    else:
      print("Please set experiment number")
      sys.exit()
    ...
    dist = tf.distributions.Beta(config.mixup_alpha, config.mixup_alpha)
    weight = dist.sample()
    if config.multiview_max_weight_for_first:
      other_weight = 1 - weight
      weight = tf.math.maximum(weight, other_weight)

    self.beta_weight = weight
    adv_final = adv_feat1 * weight + adv_feat2 * (1 - weight)

    # [N*T_obs, H, W, C]
    return tf.reshape(adv_final, [-1, H, W, C])


  def build_tower(self, scene_semantics, batch_size, grid_obs_labels,
                  grid_obs_regress, obs_length, pred_length, mixup=False):
    """Build the forward model graph.
      grid pred inputs are empty during inference
    """
    config = self.config
    # Tensor dimensions, so pylint: disable=g-bad-name
    N = batch_size
    ...
    for i, (h, w) in enumerate(config.scene_grids):
      if not config.use_grids[i]:
        ...
        continue
      # [N, T, h, w, 1]
      obs_grid_class = tf.one_hot(grid_obs_labels[i], h*w, dtype="float32")
      obs_grid_class = tf.reshape(obs_grid_class, [N, -1, h, w, 1])

      # [N, T, h, w, 2]
      obs_grid_reg = grid_obs_regress[i]
      ...
      enc_cell_obs_grid = tf.nn.rnn_cell.DropoutWrapper(
          enc_cell_obs_grid, keep_prob, dtype="float32")
      ...
      enc_cell_obs_grid_reg = tf.nn.rnn_cell.DropoutWrapper(
          enc_cell_obs_grid_reg, keep_prob, dtype="float32")

      # [N, T, h, w, 1]  ->  [N, T, h, w, h_dim]
      T = tf.shape(obs_grid_class)[1]

      if mixup and (config.multiview_exp == 3):

        # need to mixing up labels
        beta_weight = self.beta_weight
        # [N], used for the [N, M, ...] selection
        selected_extra_indices = self.selected_extra_indices
        # labels from the extra view
        # [N, M, T_obs]
        extra_view_obs_labels = self.grid_obs_labels_extra[i]
        # [N, T_obs]
        selected_extra_labels = self.gather_at_second_dim(
            extra_view_obs_labels, selected_extra_indices)
        # put the labels into one-hot then mix up
        # [N, T_obs, h*w]
        selected_extra_labels = tf.one_hot(
            selected_extra_labels, h*w, dtype="float32")
        selected_extra_labels = tf.reshape(
            selected_extra_labels, [N, -1, h, w, 1])
        obs_grid_class = beta_weight * obs_grid_class + \
            selected_extra_labels * (1 - beta_weight)

      # use the obs spatial grid to mask the convolution feature
      # [N, T, h, w, conv_dim]
      obs_grid_class_conv = tf.multiply(scene_convs[i], obs_grid_class)

      obs_grid_enc_h, obs_grid_enc_last_state = tf.nn.dynamic_rnn(
          enc_cell_obs_grid, obs_grid_class_conv,
          sequence_length=obs_length,
          dtype="float32", scope="encoder_grid_class_%d" % i)
      ...
      obs_grid_reg_enc_h, obs_grid_reg_enc_last_state = tf.nn.dynamic_rnn(
          ...
          dtype="float32", scope="encoder_grid_reg_%d" % i)
      ...
      if config.use_beam_search:
        ...
        best_beam, outputs_logits, outputs_ids, logprobs, outputs_states = \
            self.grid_decoder_beam_search(
                ...
                pred_length, dec_cell_grid,
                scale_idx=i,
                scene_convs=scene_convs,
      ...
      pred_class.append(grid_pred_decoded)
      pred_reg.append(grid_pred_reg_decoded)

    return pred_class, pred_reg


  def grid_decoder(self, first_input, enc_last_state, pred_length, rnn_cell,
                   top_scope, scope, is_conv=False, scale_idx=0,
                   scene_convs=None,
                   use_gnn=False, input_onehot=False):
    """Decoder definition. No teacher forcing."""

    # first_input -> [N, [H, W], P]
    config = self.config
    # Tensor dimensions, so pylint: disable=g-bad-name
    N = first_input.get_shape().as_list()[0]
  ...
  #
  def grid_decoder_beam_search(self, first_input, enc_last_state, pred_length,
                   rnn_cell,
                   top_scope, scope, is_conv=False, scale_idx=0,
                   scene_convs=None,
    ...
    with tf.variable_scope(scope):
      ...
      def decoder_loop_fn(time, cell_output, cell_state, loop_state):
        ...
        if cell_output is None:
          ...
          new_logprobs = tf.cond(time > config.fix_num_timestep,
                                 lambda: new_logprobs,
                                 lambda: tf.zeros([N, beam_size],
                                 dtype=tf.float32))
      ...
      output_states_ta, _, (output_grid_ids_ta,
  ...
  def gnn_edge(self, input_states, scene_convs, scale_idx=None,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as this_scope:
      if additional_scope is not None:
        return self.gnn_edge(input_states, scene_convs=scene_convs,
                             scale_idx=scale_idx,
      ...
      scene_features = scene_convs[scale_idx]
      # [N, H, W, conv_dim]
  ...
  def build_loss(self):
    ...
    for i, (h, w) in enumerate(config.scene_grids):
      ...
      if config.multiview_train and (config.multiview_exp == 3):
        # need to mixing up labels
        beta_weight = self.beta_weight
        # [N], used for the [N, M, ...] selection
        selected_extra_indices = self.selected_extra_indices
        # labels from the extra view
        # [N, M, T_pred]
        extra_view_pred_class_labels = self.grid_pred_labels_T_extra[i]
        # [N, T_pred]
        selected_extra_labels = self.gather_at_second_dim(
            extra_view_pred_class_labels, selected_extra_indices)
        # [N*T_pred]
        selected_extra_labels = tf.reshape(selected_extra_labels, [-1])
        # put the labels into one-hot then mix up
        # [N*T_pred, h*w]
        selected_extra_labels = tf.one_hot(
            tf.cast(selected_extra_labels, dtype="int32"), h*w)
        ori_labels = tf.one_hot(
            tf.cast(grid_pred_class_labels, dtype="int32"), h*w)
        mixup_labels = ori_labels * beta_weight + \
            selected_extra_labels * (1 - beta_weight)
        mixup_labels = tf.stop_gradient(mixup_labels)
        # cross entropy loss
        # [N*T_pred]
        classification_loss = tf.nn.softmax_cross_entropy_with_logits_v2(
            labels=mixup_labels,
            logits=grid_pred_decoded)
        # double weighting with adversarial weight
        if config.double_weighting:
          focal_loss_weight = tf.tile(
              tf.expand_dims(self.focal_loss_weight, axis=1),
              [1, config.pred_len])
          # [N*T_pred]
          focal_loss_weight = tf.reshape(focal_loss_weight, [-1])
          classification_loss = classification_loss * focal_loss_weight
      else:
        # one-hot label
        classification_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
  ...
  def get_feed_dict(self, batch, is_train=False):
    ...
    for j, (h, w) in enumerate(config.scene_grids):
      ...
      if not is_train and not config.use_gt_grid:
        grid_pred_reg_targets = np.zeros(
            [N, T_pred, h, w, 2], dtype="float")
        grid_pred_labels = np.zeros(
            [N, T_pred], dtype="int")
      feed_dict[self.grid_pred_regress[j]] = grid_pred_reg_targets
      feed_dict[self.grid_pred_labels_T[j]] = grid_pred_labels


      if is_train and config.multiview_train:
        M = config.multiview_max_num
        grid_obs_labels_extra = np.zeros([N, M, T_in], dtype="int")
        grid_pred_labels_extra = np.zeros([N, M, T_pred], dtype="float")
        grid_obs_reg_targets_extra = np.zeros([N, M, T_in, h, w, 2],
                                              dtype="float")
        grid_pred_reg_targets_extra = np.zeros([N, M, T_pred, h, w, 2],
                                               dtype="float")

        for i in range(len(data["extra"])):
          for k in range(len(data["extra"][i]["obs_grid_class"])):
            grid_obs_labels_extra[i, k, :] = \
                data["extra"][i]["obs_grid_class"][k][j, :]
            grid_pred_labels_extra[i, k, :] = \
                data["extra"][i]["pred_grid_class"][k][j, :]

            grid_obs_reg_targets_extra[i, k, :, :, :, :] = \
              data["extra"][i]["obs_grid_target_all_%d" % j][k][:, :, :, :]
            grid_pred_reg_targets_extra[i, k, :, :, :, :] = \
              data["extra"][i]["pred_grid_target_all_%d" % j][k][:, :, :, :]

        feed_dict[self.grid_obs_labels_extra[j]] = grid_obs_labels_extra
        feed_dict[self.grid_pred_labels_T_extra[j]] = grid_pred_labels_extra
        feed_dict[self.grid_pred_regress_extra[j]] = grid_pred_reg_targets_extra
        feed_dict[self.grid_obs_regress_extra[j]] = grid_obs_reg_targets_extra
    ...
    feed_dict[self.obs_scene] = np.squeeze(data["batch_obs_scene"])
    feed_dict[self.scene_feat] = data["batch_scene_feat"]

    if is_train and config.multiview_train:
      feed_dict[self.obs_scene_extra] = \
          np.squeeze(data["batch_extra_obs_scene"])
    if not is_train and config.multiview_train and (config.multiview_exp == 3):
      # need to feed this during validation due to the tf.conf thing
      feed_dict[self.obs_scene_extra] = np.zeros(
          (config.batch_size, config.multiview_max_num, config.obs_len),
          dtype="int32")
...
def add_div_penalty(log_probs, div_gamma, batch_size, beam_size, vocab_size):
...
def conv2d(x, out_channel, kernel, padding="SAME", stride=1,
  ...
  with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
    ...
    W = tf.get_variable("W", filter_shape, initializer=w_init, dtype="float32")
